...
% This function was used to test the performance of the Mahalanobis distance based
% minimum distance classifier using the leave-one-out method. The feature vectors
% of the healthy/pathological lung sounds should be first organized according to
% the order listed below in the vector named 's'. The healthy and pathological
% sounds' features should be saved as workspace variables in MAT files named
% healthy.mat and pathology.mat, respectively. These feature vectors can be
% generated using the class_features.m file, where the modeling error of
% each segment is added to the LPC coefficients to form the feature vector
% of that segment. The files generated at the end of this program should then be
% loaded to the memory of the DSP after making the necessary modification in the
% assembly program.
...
for	hhh=1:(60*subject_number)	% Compensate for the scaled LPCs found by DSP (exclude modeling error).
	total_sub_features(starting:ending)=[(1)*total_sub_features(starting:(ending-1));total_sub_features(ending)];
   starting=ending+1;
   ending=starting+lpc_order+1;
end	%%% Refers to hhh=1:...
