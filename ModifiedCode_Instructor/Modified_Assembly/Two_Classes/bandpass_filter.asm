;*********************************************************************************************
; Module Name: bandpass_filter.asm
;*********************************************************************************************
;
; Description: This subroutine filters the input sample in 'x1' with a FIR filter of order 256
;	by the means of the EFCOP unit. So, it initializes EFCOP for FIR stage of LEFT input
;	sample
;
; Note: The filter used here has the following specifications:
;	A FIR band pass filter (80-2,000 Hz) of order 256. The coefficients were obtained
;	using Filter Design and Analysis package of MATLABat sampling freq. of 8kHz by
;	the windowing method (boxcar window). The filter has a 'Direct form 2 transpose'
;	structure.
;
;
;
...
;		FIR_FDBA_L=the starting address of the left channel taps
;	Output:
...
bandpass_filter
		...
		movep   #$000,y:M_FACR                  ; No coeff. scaling applied

		movep   #$1C1,y:M_FCSR                  ; Enable EFCOP
