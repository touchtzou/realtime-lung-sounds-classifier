function [a,e,k] = modified_aryule( x, p) % Modified from MATLAB's original
														% aryule() function
...
R = xcorr(x,p,'coeff'); % Modified from 'biased' to 'coeff' so that similar results,
								% with the ones calculated by the DSP, are obtained.
